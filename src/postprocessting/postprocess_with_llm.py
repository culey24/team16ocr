...
def call_gemini(prompt: str, api_key: str) -> Optional[Dict]:
    ...
    try:
        ...
        resp.raise_for_status()
        j = resp.json()
        output = j["candidates"][0]["content"]["parts"][0]["text"].strip()
        # Parse output: assume LLM outputs "Corrected: [text]\nConfidence: [float]"
        lines = output.splitlines()
        suggested = lines[0].replace("Corrected: ", "").strip() if lines else ""
        confidence_str = lines[1].replace("Confidence: ", "").strip() if len(lines) > 1 else "0.5"
        confidence = float(confidence_str) if confidence_str.replace('.', '', 1).isdigit() else 0.5
        return {"suggested": suggested, "confidence": confidence, "raw": j}
    except Exception as e:
        print("Gemini call failed:", e, file=sys.stderr)
        return None
...
def call_openai(prompt: str, api_key: str) -> Optional[Dict]:
    ...
    try:
        ...
        openai.api_key = api_key
        # Use chat completion
        system = "You are a helpful assistant that corrects OCR'd text. Only output the corrected sentence and confidence."
        messages = [{"role":"system","content":system}, {"role":"user","content":prompt}]
        resp = openai.ChatCompletion.create(model="gpt-4o-mini", messages=messages, temperature=0.05, max_tokens=300)
        output = resp["choices"][0]["message"]["content"].strip()
        # Parse output: assume "Corrected: [text]\nConfidence: [float]"
        lines = output.splitlines()
        suggested = lines[0].replace("Corrected: ", "").strip() if lines else ""
        confidence_str = lines[1].replace("Confidence: ", "").strip() if len(lines) > 1 else "0.5"
        confidence = float(confidence_str) if confidence_str.replace('.', '', 1).isdigit() else 0.5
...
def build_prompt_for_llm(text: str) -> str:
    # Generalize for mixed languages (Korean/English)
    return (
        f"The following is OCR-recognized text (may contain Korean and English):\n\"{text}\"\n\n"
        "Task:\n"
        "1) Correct spelling and spacing.\n"
        "2) Fix OCR character recognition errors if possible.\n"
        "3) Do not change numbers, dates, or proper nouns.\n"
        "4) Respect the original text as much as possible.\n"
        "Output format:\n"
        "Corrected: [corrected sentence only]\n"
        "Confidence: [your confidence score as float between 0.0 and 1.0]\n"
    )
